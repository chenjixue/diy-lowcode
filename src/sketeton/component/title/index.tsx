...
import { createIcon } from "../../../util/create-icon.tsx";
import './title.less';
import { IPublicTypeI18nData } from '@/plugin-manager/plugin-manager.ts';
...
export class Title extends Component<IPublicTypeTitleProps> {
    ...
    renderLabel = (label) => {
        return (
            <span className="lc-title-txt">{label}</span>
        );
    }
        ...
        return (
            ...
            >
                {icon ? <b className="lc-title-icon">{icon}</b> : null}
                 {_title.label ? this.renderLabel(_title.label["zh-CN"]): null}
            </span>
        );
    }
}
